...
assert(riskMigration.includes("elsif term_record.flag = 'PROFANITY' then") && riskMigration.includes("score := score + 0") && riskMigration.includes("fuck you") && riskMigration.includes("this artist is fucking trash"), "Risk classifier must keep standalone profanity low while preserving targeted profanity as medium-risk abuse.");
...
assert(riskMigration.includes("char_length(clean_body) > 250") && riskMigration.includes("clean_body ~ '(.)\\1{24,}'") && riskMigration.includes("clean_body ~ '[!?.,]{40,}'") && !riskMigration.includes("EXCESSIVE_PUNCTUATION") && !riskMigration.includes("EXCESSIVE_UPPERCASE") && !riskMigration.includes("LONG_UNBROKEN_TOKEN") && !riskMigration.includes("EXCESSIVE_MENTIONS") && !riskMigration.includes("position(U&'\\200D'"), "Risk classifier must keep normal elongated fan excitement, caps, keyboard-smashes, emoji, mentions, and punctuation low.");
...
assert(linkPromotionMigration.includes("char_length(clean_body) > 250") && linkPromotionMigration.includes("clean_body ~ '(.)\\1{24,}'") && linkPromotionMigration.includes("clean_body ~ '[!?.,]{40,}'") && !linkPromotionMigration.includes("EXCESSIVE_PUNCTUATION") && !linkPromotionMigration.includes("EXCESSIVE_UPPERCASE") && !linkPromotionMigration.includes("LONG_UNBROKEN_TOKEN") && !linkPromotionMigration.includes("EXCESSIVE_MENTIONS") && !linkPromotionMigration.includes("position(U&'\\200D'"), "Link-promotion safety migration must preserve normal fan excitement and emoji composition as low-risk behavior.");
