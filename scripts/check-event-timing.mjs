...
assert(publicPage.includes('variant === "join" && (') && publicPage.includes('border: "1px solid rgba(0,255,65,0.22)"'), "The full chat state must render a bordered content area and no full-state button.");
...
assert(!publicPage.includes("window.location.reload()") && !publicPage.includes("Refresh"), "The full chat state must not render or handle a refresh button.");
...
assert(publicPage.includes("joinedIdentity ? (") && publicPage.includes("<LiveMessageStream messages={messages}") && publicPage.includes("<JoinChatPanel eventId={eventId}") && publicPage.includes("eventTitle={title}") && publicPage.includes("listenerCount={listenerCount}"), "Live chat should have one flow: join panel before identity, active stream after identity.");
