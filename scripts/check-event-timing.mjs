...
assert(!publicPage.includes("listenerCount") && !publicPage.includes("listeners:"), "Public live chat must not render or manage the removed listener count.");
assert(publicPage.includes('type JoinChatPanelVariant = "join" | "full"') && publicPage.includes("live chat is full.") && publicPage.includes("but you can continue listening to ${trimmedEventTitle} <3.") && publicPage.includes("but you can continue listening <3."), "Join Chat panel must support the permanent-capacity full state with lowercase copy and no separate layout.");
assert(publicPage.includes('variant === "join" && !checkingAdmission') && publicPage.includes('border: "1px solid rgba(0,255,65,0.22)"'), "The full chat state must render a bordered content area and no full-state button.");
assert(publicPage.includes("function isLocalChatFullPreviewEnabled") && publicPage.includes("import.meta.env.PROD") && publicPage.includes("previewChatFull") && publicPage.includes("previewFull={isLocalChatFullPreviewEnabled()}"), "The Live Chat Full preview flag must be local-development only and reuse the existing join panel.");
assert(!publicPage.includes("window.location.reload()") && !publicPage.includes('buttonLabel: "Refresh"') && !publicPage.includes(">Refresh<"), "The full chat state must not render or handle a refresh button.");
...
assert(!publicPage.includes("onListenerCount") && !publicPage.includes("setListenerCount"), "Listener-count setter plumbing must be removed from the public live chat flow.");
...
assert(publicPage.includes("joinedIdentity ? (") && publicPage.includes("<LiveMessageStream messages={messages}") && publicPage.includes("<JoinChatPanel eventId={eventId}") && publicPage.includes("eventTitle={title}") && !publicPage.includes("listenerCount={listenerCount}"), "Live chat should have one flow without listener-count UI coupling.");
...
assert(chatHook.includes("realtimeStatusRef") && chatHook.includes('status === "SUBSCRIBED"') && chatHook.includes('scheduleRecovery("subscribed")'), "Public chat must perform an immediate delta recovery after the realtime channel subscribes.");
assert(chatHook.includes('status === "CHANNEL_ERROR"') && chatHook.includes('status === "TIMED_OUT"') && chatHook.includes('status === "CLOSED"') && chatHook.includes("realtime_reconnect_scheduled"), "Public chat must handle realtime disconnect states with bounded reconnect scheduling.");
assert(chatHook.includes('scheduleRecovery("visibilitychange")') && chatHook.includes('scheduleRecovery("focus")') && chatHook.includes('scheduleRecovery("pageshow")') && chatHook.includes('scheduleRecovery("online")'), "Public chat must recover immediately when mobile Safari returns to the foreground or network comes online.");
assert(chatHook.includes('document.visibilityState === "hidden"') && chatHook.includes("poll_skipped_hidden"), "Public chat fallback polling must not keep polling normally while the page is hidden.");
assert(chatHook.includes("delta_fetch_skipped_in_flight") && chatHook.includes("pendingDeltaRef.current = true"), "Public chat must coalesce recovery wakeups while a delta fetch is in flight.");
assert(chatHook.includes("chatDebug") && chatHook.includes("performance.now()") && chatHook.includes("visibility: document.visibilityState") && chatHook.includes("online: navigator.onLine"), "Public chat must include development-only delivery timing diagnostics without message content.");
...
assert(!chatService.includes('supabase.rpc("get_event_listener_count"') && !chatService.includes("getEventListenerCount"), "Public chat must not fetch the removed listener-count RPC.");
...
assert(chatService.includes("joinEventChatIdentity") && chatService.includes("getOrCreateChatSessionId") && chatService.includes('supabase.rpc("join_event_chat"') && chatService.includes('supabase.rpc("get_chat_admission_status"') && !chatService.includes("join_event_chat_test") && !chatService.includes("get_chat_admission_status_test"), "Visitor chat names and admission checks must use only the production chat admission RPCs.");
assert(chatService.includes("ChatJoinError") && chatService.includes("CHAT_CAPACITY_FULL") && chatService.includes("chat_capacity_full"), "Join service must map permanent-capacity admission failures to a typed UI state.");
...
assert(!chatSchema.includes("create or replace function public.get_event_listener_count") && !chatSchema.includes("grant execute on function public.get_event_listener_count(uuid)"), "Canonical chat schema must not expose the removed listener-count RPC.");
...
assert(chatSchema.includes("1000 + greatest") && chatSchema.includes("floor(extract(epoch from (now() - event_started_at)) / 60)") && chatSchema.includes("chat-capacity:") && chatSchema.includes("chat_capacity_full"), "Join RPC must enforce permanent dynamic capacity for new event/session participants.");
assert(chatSchema.includes("if existing_participant.id is null then") && chatSchema.indexOf("if existing_participant.id is null then") < chatSchema.indexOf("raise exception 'chat_capacity_full'"), "Join RPC capacity checks must not block already admitted event/session participants.");
const obsoleteParticipantActivityColumn = "last_" + "seen_at";
assert(chatSchema.includes("inserted_participant.joined_at") && !chatSchema.includes(obsoleteParticipantActivityColumn), "Join RPC must return the trusted joined_at cutoff without obsolete activity tracking.");
...
assert(listenerCutoffMigration.includes("create or replace function public.get_event_listener_count") && listenerCutoffMigration.includes("count(*)::integer"), "Historical listener/cutoff migration remains intact for replay history.");
