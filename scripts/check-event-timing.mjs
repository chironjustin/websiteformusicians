...
assert(chatSchema.includes("grant execute on function public.submit_chat_message(uuid, uuid, uuid, text, uuid) to anon, authenticated") && chatSchema.includes("revoke all on function public.submit_chat_message(uuid, uuid, text, uuid) from anon, authenticated"), "Only the session-verified visitor submit RPC signature should be executable by public visitors.");
...
assert(queueMigration.includes("grant execute on function public.submit_chat_message(uuid, uuid, uuid, text, uuid) to anon, authenticated") && queueMigration.includes("revoke all on function public.submit_chat_message(uuid, uuid, text, uuid) from anon, authenticated"), "Visitor submit RPC execute grant must remain only on the session-verified signature.");
...
assert(linkPromotionMigration.includes("grant execute on function public.submit_chat_message(uuid, uuid, uuid, text, uuid) to anon, authenticated") && linkPromotionMigration.includes("revoke all on function public.submit_chat_message(uuid, uuid, text, uuid) from anon, authenticated"), "Link-promotion safety migration must preserve visitor submit access only on the session-verified signature.");
