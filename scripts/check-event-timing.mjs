...
assert(chatHook.includes('mode !== "public"') && chatHook.includes("window.setInterval") && chatHook.includes("loadMessages(false)"), "Public chat must safely refetch through the cutoff-aware RPC even when direct table reads are not exposed.");
...
assert(chatService.includes('supabase.rpc("submit_chat_message"') && chatService.includes("p_client_token") && chatService.includes("get_visitor_visible_chat_messages"), "Visitor submissions must use the scoped public RPC and joined visitors must load cutoff-aware approved plus sender-private messages.");
