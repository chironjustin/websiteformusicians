...
assert(riskMigration.includes("elsif term_record.flag = 'PROFANITY' then") && riskMigration.includes("score := score + 5") && riskMigration.includes("fuck you") && riskMigration.includes("this artist is fucking trash"), "Risk classifier must keep standalone profanity low while preserving targeted profanity as medium-risk abuse.");
...
assert(!riskMigration.includes("REPEATED_CHARACTERS") && !riskMigration.includes("EXCESSIVE_PUNCTUATION") && !riskMigration.includes("position(U&'\\200D'"), "Risk classifier must treat repeated letters, emoji sequences, ZWJ emoji composition, and enthusiastic punctuation as normal low-risk fan behavior.");
...
assert(!linkPromotionMigration.includes("REPEATED_CHARACTERS") && !linkPromotionMigration.includes("EXCESSIVE_PUNCTUATION") && !linkPromotionMigration.includes("position(U&'\\200D'"), "Link-promotion safety migration must preserve normal fan excitement and emoji composition as low-risk behavior.");
assert(linkPromotionMigration.includes("fan_excitation_classifier_repair") && linkPromotionMigration.includes("'PROFANITY'") && linkPromotionMigration.includes("'SUSPICIOUS_INVISIBLE_CHARACTERS'"), "Link-promotion safety migration must reclassify old non-public profanity-only and emoji-invisible false positives.");
