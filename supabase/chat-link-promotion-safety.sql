...
with candidates as (
  ...
  where status in ('pending', 'queued')
    ...
    and (
      ...
      or public.normalize_chat_body_for_risk(body) ~ '(follow me|follow my|follow @[a-z0-9_\.]|subscribe to me|subscribe to my|dm me|message me|contact me|add me|add me on|send me a dm|hit me up|join my|my insta is|my instagram is|my ig is|my snap is|my snapchat is|my telegram is|instagram|insta|ig|tiktok|tik tok|twitter|snapchat|snap|discord|telegram|twitch|youtube|yt|soundcloud|spotify|facebook|fb).*@[a-z0-9]'
    )
), reclassified as (
  update public.chat_messages
  set
    risk_level = classification.result->>'riskLevel',
    risk_score = (classification.result->>'riskScore')::integer,
    risk_flags = coalesce(ARRAY(SELECT jsonb_array_elements_text(classification.result->'riskFlags')), array[]::text[]),
    classified_at = clock_timestamp(),
    classifier_version = classification.result->>'classifierVersion',
    auto_publish_eligible = false,
    status = case when classification.result->>'riskLevel' = 'high' then 'rejected' else 'pending' end,
    queued_at = null,
    published_at = null,
    rejected_at = case when classification.result->>'riskLevel' = 'high' then clock_timestamp() else rejected_at end,
    rejection_source = case when classification.result->>'riskLevel' = 'high' then 'automatic_rules' else rejection_source end,
    rejection_reason = case when classification.result->>'riskLevel' = 'high' then 'automatic_rules' else rejection_reason end,
    last_queue_error = case when candidates.status = 'queued' then 'queue_eligibility_revoked' else last_queue_error end
  from candidates
  cross join lateral (
  ...
  where chat_messages.id = candidates.id
  returning chat_messages.*, candidates.status as previous_status
)
insert into public.chat_message_moderation_audit (
  event_id,
  message_id,
...
select
  ...
  null,
  case when reclassified.previous_status = 'queued' then 'queue_eligibility_revoked' else 'message_classified' end,
