...
-- access for the event lifetime. New participants are admitted only while the
-- current dynamic capacity has room:
--
--   1000 + floor(minutes since event starts_at)
--
...
create or replace function public.get_chat_admission_status(
  p_event_id uuid,
  p_session_id uuid
)
returns jsonb
...
begin
  ...
  return jsonb_build_object(
    'already_joined', existing_participant.id is not null,
    'chat_full', existing_participant.id is null and admitted_count >= current_capacity,
    'admitted_count', admitted_count,
    'current_capacity', current_capacity,
    'participant_id', existing_participant.id,
    'session_id', existing_participant.session_id,
...
revoke all on function public.get_chat_admission_status(uuid, uuid) from public;
grant execute on function public.get_chat_admission_status(uuid, uuid) to anon, authenticated;

notify pgrst, 'reload schema';

-- Verification:
--
-- select count(*) as admitted_users
-- from public.event_chat_participants
-- where event_id = '<EVENT_ID>';
--
-- select
--   events.id,
--   1000 + greatest(0, floor(extract(epoch from (now() - events.starts_at)) / 60)::integer) as current_capacity
-- from public.events
-- where id = '<EVENT_ID>';
--
-- Existing event/session participant should still join even when admitted_users
-- is already equal to current_capacity.
--
-- A new event/session participant should receive chat_capacity_full when
-- admitted_users >= current_capacity.
